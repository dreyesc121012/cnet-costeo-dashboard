...
st.set_page_config(page_title="CNET Dashboard", layout="wide")
...
def render_financial_dashboard():
    ...
    if "token_result_financial" not in st.session_state:
        if qp.get("code"):
            try:
            ...
            if "access_token" in result:
                st.session_state.token_result_financial = result
                _clear_query_params()
                st.rerun()
            else:
    ...
    token_result = st.session_state.token_result_financial
    ...
    with colA:
        if st.button("🔄 Refresh data", key="refresh_financial"):
            st.session_state.pop("excel_bytes_financial", None)
    ...
    try:
        if "excel_bytes_financial" not in st.session_state:
            st.info("📥 Downloading Excel from SharePoint/OneDrive…")
            st.session_state.excel_bytes_financial = download_excel_bytes_from_shared_link(
                token_result["access_token"],
                ONEDRIVE_SHARED_URL
            )
        excel_bytes = st.session_state.excel_bytes_financial
    except Exception as e:
    ...
    target = st.slider("Target margin (%)", 0, 60, 25, key="target_margin") / 100
    yellow_zone = 0.05
...
# ============================================================
def main():
    st.sidebar.title("CNET Dashboard")
    ...
    elif section == "Invoices Control":
        invoices_control_app.app()
